...
%      H = FACERECO
%
%      FACERECOGNIwTIONTOOL('CALLBACK',hObject,eventData,handles,...) calls the local
...
K=3;
noIt=2;
%A=struct2array(load('dictionary.mat'));
%A=normalizeColumns(A);
%Test_Image_Down_Sampled = double(imresize(Test_Image,[m1 n1]));
%y = Test_Image_Down_Sampled(:);
%Test_Image_Down_Sampled = double(imresize(Test_Image,[114 90]));
%y = Test_Image_Down_Sampled;
y = double(Test_Image);
y = sliding(double(y));
lines = size(y,1);
D_hat=initialize(lines,K,y);
[S_hat Dict]=modDic(D_hat,y,noIt);
y = Dict(:);
n = size(A,2);
%  fprintf('Processing .... \n')
